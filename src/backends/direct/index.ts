...
import { CodexErrandError, isAbortError } from "../../errors.js";
...
function mapHttpError(status: number, body: string): CodexErrandError {
  const detail = body.slice(0, 500);
  if (status === 401 || status === 403) {
    return new CodexErrandError("DIRECT_AUTH_FAILED", `Direct authentication failed with HTTP ${status}`, {
      details: detail,
    });
  }
  if (status === 429) {
    return new CodexErrandError("DIRECT_RATE_LIMITED", "Direct backend is rate limited", {
      retryable: true,
      details: detail,
    });
  }
  if (status >= 500) {
    return new CodexErrandError("DIRECT_SERVER_ERROR", `Direct backend returned HTTP ${status}`, {
      retryable: true,
      details: detail,
    });
  }
  if (status === 400) {
    return new CodexErrandError("DIRECT_REQUEST_REJECTED", "Direct backend rejected the request", {
      details: detail,
    });
  }
  return new CodexErrandError("DIRECT_HTTP_ERROR", `Direct backend returned HTTP ${status}`, {
    retryable: status === 0,
    details: detail,
  });
}

function normalizeDirectError(error: unknown): CodexErrandError {
  ...
  return new CodexErrandError("DIRECT_TRANSPORT_ERROR", message, { retryable, cause: error });
}
...
async function sendOnce(
  ...
  try {
    ...
    if (!parsed.text && !parsed.image) {
      throw new CodexErrandError("DIRECT_EMPTY_RESPONSE", "Direct backend returned an empty response", {
        retryable: true,
      });
    }
    return parsed;
...
export async function executeDirectText(taskId: string, options: TextOptions): Promise<DirectTextExecution> {
  ...
  if (!response.text) {
    throw new CodexErrandError("DIRECT_EMPTY_TEXT", "Direct backend returned no text", { retryable: true });
  }
  ...
  const worker = async (workerContext: typeof context): Promise<void> => {
    while (true) {
      ...
      try {
        ...
        if (!response.image) {
          throw new CodexErrandError("DIRECT_EMPTY_IMAGE", "Direct backend returned no image", {
        ...
        const normalized = normalizeDirectError(error);
        throw new CodexErrandError(normalized.code, normalized.message, {
