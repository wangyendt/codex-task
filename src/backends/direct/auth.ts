...
function loadAuthDocument(codexHome: string): Record<string, unknown> {
  ...
  if (!existsSync(path)) {
    throw new CodexErrandError("AUTH_NOT_FOUND", `No Codex login found at ${path}. Run codex login first.`, {
      exitCode: 2,
    });
  }
  try {
    ...
    throw new CodexErrandError("AUTH_INVALID", `Could not parse ${path}`, { exitCode: 2, cause: error });
  }
}
...
function tokensFromDocument(document: Record<string, unknown>): AuthTokens {
  ...
  if (!tokens?.access_token) {
    throw new CodexErrandError("AUTH_INVALID", "Codex auth.json has no OAuth access token. Run codex login.", {
      exitCode: 2,
    });
  }
  return {
...
async function refreshTokens(refreshToken: string, proxy?: string): Promise<AuthTokens> {
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new CodexErrandError("AUTH_REFRESH_FAILED", `Codex token refresh failed with HTTP ${response.status}`, {
...
export async function ensureDirectAuth(
  ...
  if (!initial.refresh_token) {
    throw new CodexErrandError("AUTH_EXPIRED", "Codex OAuth token expired and has no refresh token. Run codex login.", {
      exitCode: 2,
    });
  }

  const path = authPath(codexHome);
  return withFileLock(`${path}.codexerrand.lock`, async () => {
    ...
    if (!refreshed.access_token) {
      throw new CodexErrandError("AUTH_REFRESH_FAILED", "Codex token refresh returned no access token");
    }
