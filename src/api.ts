...
import { asCodexErrandError, CodexErrandError, isAbortError, usageError } from "./errors.js";
...
async function executeSdkImage(taskId: string, options: ImageOptions): Promise<SdkExecution> {
  ...
  for (const target of expected) {
    if (!existsSync(target.path)) {
      throw new CodexErrandError("SDK_IMAGE_MISSING", `Codex SDK did not create ${target.path}`);
    }
    artifacts.push({
      path: target.path,
      kind: "image",
...
function completedFromSdk(taskId: string, execution: SdkExecution, effectiveModel?: string): CompletedResult {
  if (execution.status !== "completed") {
    throw new CodexErrandError("SDK_TASK_INCOMPLETE", execution.text || "SDK task did not complete", {
